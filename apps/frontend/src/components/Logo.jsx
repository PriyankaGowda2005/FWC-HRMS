...
const Logo = ({ 
  ...
  const LogoContent = () => (
    <div className={`flex items-center group transition-all duration-300 hover:scale-105 ${className}`}>
      ...
      >
        {/* FWC Logo Image with black filter */}
        <img 
          src="/logo.png" 
          alt="FWC Logo" 
          ...
          onError={(e) => {
            ...
            if (e.target.src.includes('.png')) {
              ...
              // If all images fail, show fallback black logo
              e.target.style.display = 'none'
              e.target.nextSibling.style.display = 'flex'
            }
          }}
        />
        {/* Fallback FWC black logo */}
        <div 
          className="w-full h-full bg-black rounded-xl flex items-center justify-center shadow-lg"
          style={{ display: 'none' }}
        >
